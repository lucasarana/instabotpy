...
class InstaBot:
        ...
        if (self.login_status):
            ...
            if self.login_status == 1:
                ...
                try:
                    ...
                    self.end_cursor = all_data['tag']['media']['page_info']['end_cursor']
                except:
                    self.media_by_tag = []
    ...
    def new_auto_mod(self):
        while True:
            ...
            if self.this_tag_like_count % 10 == 0:
                self.write_log("Entramos y cambiamos de URL")
                url_tag = self.url_tag % (self.tag_list[0]) + '&max_id=' + self.end_cursor
                r = self.s.get(url_tag)
                all_data = json.loads(r.text)
                time.sleep(70)
    ...
    def new_auto_mod_like(self):
        if time.time() > self.next_iteration["Like"] and self.like_per_day != 0 \
            ...
            self.write_log("Contador de Tags %s" % self.this_tag_like_count)
